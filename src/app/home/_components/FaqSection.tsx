...
export function FaqSection() {
  ...
  return (
    <section className="relative w-full bg-white flex flex-col items-center py-24">
      {/* Header */}
      ...
      <h2 className="text-4xl md:text-[56px] font-paragraph font-normal text-cs-ink leading-[1.1] text-center mb-16">
        Frequently Asked <span className="text-brand-default">Questions.</span>
      </h2>

      {/* FAQ Accordion */}
      <div className="w-[90%] xl:w-[82%] max-w-[1260px] mx-auto flex flex-col md:flex-row gap-4 md:gap-5 h-[800px] md:h-[600px]">
        {faqs.map((faq, index) => {
          ...
          return (
            <div
              ...
              className={`relative rounded-[32px] overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col cursor-pointer
                ${isActive
                  ? "flex-1 md:flex-[2.1] bg-gradient-to-t from-[#2384C6] to-[#0040C1] text-white"
                  : "h-[80px] md:h-auto flex-none md:flex-1 bg-[#F5FAFF] text-neutral-900 hover:bg-[#eaf3ff]"
                }
              `}
            >
              <div className={`flex flex-col h-full ${isActive ? "p-6 md:p-10" : "p-5 md:px-6 md:py-8"}`}>

                {/* Card Header */}
                <div className={`flex justify-between items-center ${isActive ? "mb-6 md:mb-8 text-white" : "mb-0 md:mb-8 text-neutral-900"}`}>
                ...
                {isActive ? (
                  <div className="flex flex-col flex-1 h-full animate-in fade-in duration-500 text-white">
                    ...
                    <p className="text-lg font-paragraph font-normal leading-relaxed mb-8 flex-1 pr-4">
                      {faq.answer}
                    </p>
                  ...
                  <>
                    <div className="hidden md:flex flex-1 items-end justify-start pb-8 overflow-hidden">
                      <span
                    ...
                    <div className="md:hidden mt-1 text-sm font-paragraph font-normal text-neutral-900 truncate max-w-[90%]">
                      {faq.question}
                    </div>
                  </>
                )}
